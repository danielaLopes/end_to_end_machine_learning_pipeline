...
def create(
    ...
    try:
        feature_views = fs.get_feature_views(name=f"{settings.SETTINGS["FS_VIEW_NAME"]}")
    except hsfs.client.exceptions.RestAPIError:
        logger.info(f"No feature views found for {settings.SETTINGS["FS_VIEW_NAME"]}.")
    ...
    predict_cats_dogs_fg = fs.get_feature_group(
        settings.SETTINGS["FS_PROJECT_NAME"], version=feature_group_version
    )
    ds_query = predict_cats_dogs_fg.select_all()
    feature_view = fs.create_feature_view(
        name=f"{settings.SETTINGS["FS_VIEW_NAME"]}",
        description="Cats or dogs forecasting model.",
        query=ds_query,
        labels=[],
    )
    ...
    feature_view.create_training_data(
        description="Predict cat or dog training dataset",
        data_format="csv",
    ...
    # Save metadata.
    metadata = {
        "feature_view_version": feature.version,
